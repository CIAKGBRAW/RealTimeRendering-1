...
void initialize(void)
{
	...
	const GLchar * vertexShaderShaderCode =
		...
		"uniform int u_fraglighting_enabled;" \
		...
		"out vec3 light_directionRedFrag;" \
		"out vec3 light_directionGreenFrag;" \
		...
		"light_directionRedFrag = vec3(u_light_positionRed) - eye_coordinates.xyz;" \
		"light_directionGreenFrag = vec3(u_light_positionGreen) - eye_coordinates.xyz;" \
	...
	fprintf(gpFile, "After vertex shader\n");
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"in vec3 light_directionRedFrag;" \
		"in vec3 light_directionGreenFrag;" \
		...
		"FragColorRed = vec4(phong_ads_colorRedVer,1.0);" \
		"FragColorGreen = vec4(phong_ads_colorGreenVer,1.0);" \

		"FragColor = FragColorBlue + FragColorRed + FragColorGreen;" \
		"}"\
		"else if(u_fraglighting_enabled == 2)" \
		"{" \
		...
		"FragColorBlue = vec4(phong_ads_color2,1.0);" \

		"vec3 normalized_light_directionRed=normalize(light_directionRedFrag);" \
		"vec3 ambientRed = u_LaRed * u_KaRed;" \
		...
		"vec3 normalized_light_directionGreen=normalize(light_directionGreenFrag);" \
		"vec3 ambientGreen = u_LaGreen * u_KaGreen;" \
	...
	fprintf(gpFile, "After fragment shader\n");
...
void display()
{
	...
	else if (gbFLight == true)
	{

		glUniform1i(F_KeyPressed_uniform, 2);
...
//	myArrGreen[0] = lightPositionGreen[0] * sin(angleRadian);
